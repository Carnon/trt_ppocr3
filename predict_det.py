...
class BaseModel(nn.Module):
    def __init__(self):
        ...
        self.backbone = MobileNetV3(in_channels=3, scale=0.5, model_name="large", disable_se=True)
...
def infer(img_path):
    ...
    ckt = torch.load("weights/ch_ptocr_v3_det_infer.pth", map_location="cpu")
...
def export_onnx():
    ...
    x = torch.randn(size=(1, 3, 960, 960))
    torch.onnx.export(
        base_model,
        x,
        "weights/ch_det.onnx",
...
def infer_onnx(img_path):
    ...
    sess = rt.InferenceSession("weights/ch_det.onnx")
